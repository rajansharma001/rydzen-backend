...
import { sendVerification } from "../../middlewares/sendVerificationMail";
...
export const singUpController = async (req: Request, res: Response) => {
  try {
    ...
    console.error("Bad Request for signup.", error);
    return res.status(500).json({ error: "Bad Request for signup." });
  }
};
