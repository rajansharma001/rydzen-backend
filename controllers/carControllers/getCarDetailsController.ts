...
export const getCarDetailsByIdController = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const getCarDetailsById = await CarDetails.findById(_id);
    if (!getCarDetailsById || getCarDetailsById.length === 0) {
      return res.status(400).json({ error: "Car details not found." });
    }

    return res.status(200).json({
      success: "Car details fetched Successfully.",
      getCarDetailsById,
    });
  } catch (error) {
    console.log("Bad Request for getting car details.", error);
